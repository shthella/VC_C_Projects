...
char *ip_address = NULL; // Declaration of ip_address variable
...
void admin_interface(int client_socket) {
  ...
  do {
    ...
    if (isdigit(message[0])) {
      ...
      case 3:
      	debug_log(3, "Call for the admin sign up\n");
      	int restaurant_id;
      	char message[1024];
      	printf("Enter Restaurant ID:");
      	fgets(mesage, sizeof(message), stdin);
      	send(client_socket, message, strlen(message), 0);
      	
      	char username[50], password[50];
      	debug_log(3, "Prompt the admin for username\n");
      	printf("Enter desired username of admin: ");
      	fgets(username, sizeof(username), stdin);
      	username[strcspn(username, "\n")] = '\0'; // Remove newline character
      	
      	// Send username to server
      	debug_log(3, "Sending username to server for authentication\n");
      	send_message(client_socket, username);
      	
      	// Read password
      	debug_log(3, "Prompt the admin for password\n");
      	printf("Enter desired password of admin: ")
      	fgets(password, sizeof(password), stdin);
      	password[strcspn(password, "\n")] = '\0'; // Remove newline character
      	
      	// Send password to server
      	debug_log(3, "Sending password to server for authentication\n");
  ...
  retry:
      ...
      // Send username to server
      debug_log(3, "Send username to server\n");
      send_message(client_socket, username);
      ...
      // Send password to server
      debug_log(3, "Send password to server\n");
      send_message(client_socket, password);
...
int main(int argc, char * argv[]) {
	printf("Enter the server ip address to connect\n");
  if (argc < 2 || strcmp(argv[1], "-ip") != 0) {
        setIP();
    } else {
        // IP address provided as a command-line argument
        if (argc != 3) {
        	printf("Wrong entry format of ip address\n");
